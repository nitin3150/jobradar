...
async def run_job_scrape_pipeline(
    ...
    for company, raw_jobs in fetched:
        for raw in raw_jobs:
            ...
            # score_job calls llm_complete synchronously (sync litellm.completion
            # under the hood). With 100+ jobs per run that would freeze the
            # asyncio loop for minutes, so offload to a worker thread and await.
