...
Primary provider is `settings.llm_provider`; on any exception during the
primary call the wrapper falls back to `settings.llm_fallback_provider`.
Each entry in `PROVIDERS` below carries its own model + key + optional api_base.
"""
