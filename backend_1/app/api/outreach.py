...
async def generate_outreach(
    ...
    """Generate a personalized outreach message using the canonical LLM client."""
    ...
    # Generate with the canonical LLM client, fallback on error.
    # llm_complete is synchronous; offload to a worker thread so the FastAPI
    # handler doesn't stall the event loop waiting on the API.
    #
    # temperature=1.0 preserves pre-consolidation behavior — the legacy
    # `call_claude` wrapper did not set temperature, so litellm used its
    # default (~1.0 for OpenAI-compat endpoints). Outreach emails are
    # creative writing; lower temperatures make them formulaic. The lower
    # default of `llm_complete` is fine for deterministic JSON (scorer.py),
    # so we explicitly override here.
    try:
